// ============================================================
//  cv-light.typ — Joys Di Giorgio | Software Developer CV (ATS)
// ============================================================

// ---------- CONFIG ----------
#let accent   = rgb("#0969da")
#let surface  = rgb("#f6f8fa")
#let border   = rgb("#d0d7de")
...
#set text(font: "Consolas", size: 9pt, fill: tc)
...
#let section-title(t) = {
  v(0.6em)
  text(size: 9pt, fill: accent, weight: "bold", upper(t))
...
#let edu-entry(degree: "", school: "", period: "", body) = {
  grid(
    columns: (1fr, auto),
    [
      #text(weight: "bold", fill: tc, degree)
      #linebreak()
...
#let skill-row(name, level) = {
  grid(
    columns: (0.5fr, 1fr),
    column-gutter: 6pt,
    align(horizon, text(fill: tc, size: 8.5pt, name)),
...
#let staerke-item(t) = {
  stack(dir: ltr, text(fill: accent, "▸ "), text(fill: tc, size: 8.5pt, t))
  v(0.25em)
}
...
#block(
  ...
  #grid(
    ...
    column-gutter: 16pt,
    align(horizon)[
      #text(size: 22pt, weight: "bold", fill: tc, fullname)
      #linebreak()
...
#grid(
  ...
  #section-title("Berufserfahrung")
    ...
    - Erweiterung eines Projektmanagementsystems um externe Projekte, Drag-and-Drop-Planung und mehrstufige Hierarchien
    #v(0.2em)
    ...
    - Entwicklung eines Produktionsautomatisierungstools (Gerätescan, One-Click-Konfiguration)
    ...
    - 4-jährige Berufsausbildung — Fundament für tiefes technisches Systemverständnis
  ]
  ...
  #colbreak()
    ...
    Vollautomatischer Orderflow-Scalping-Bot für Binance Futures. Double DQN + LSTM kombiniert mit klassischen Indikatoren (VWAP, CVD, Order-Book).
    #v(0.2em)
  ...
  *Tools* \
  ...
  #grid(
    ...
    column-gutter: 8pt,
    row-gutter: 0.5em,
    text(fill: tc, "Deutsch"),    text(fill: muted, "Muttersprache"),
    text(fill: tc, "Englisch"),   text(fill: muted, "Beruflich"),
    text(fill: tc, "Italiano"),   text(fill: muted, "Grundkenntnisse"),
  )
